            ...
            {[
              ...
              return (
                <button
                  ...
                  className={`px-4 sm:px-6 py-2 rounded-xl text-sm sm:text-base transition-colors ${active ? 'bg-black text-white' : 'bg-transparent text-black hover:bg-gray-100'
                    }`}
                >
                  {opt.label}
                </button>
                  ...
                  >
                    {carouselItems.map((item) => (
                      <SwiperSlide key={item.id}>
                        <div onClick={() => openPost(item)} className="w-full h-56 sm:h-64 md:h-80 bg-gray-50 flex items-center justify-center cursor-pointer group relative">
                          <Image src={item.image} alt={item.title} fill className="object-contain p-2 transition-transform duration-300 group-hover:scale-[1.02]" />
                        </div>
                      </SwiperSlide>
                    ))}
                  </Swiper>
              ...
              {verticalCards.map((post) => (
                <article key={post.id} onClick={() => openPost(post)} className="group bg-white rounded-2xl overflow-hidden shadow-[0_4px_16px_rgba(0,0,0,0.06)] cursor-pointer">
                  <div className="relative w-full h-44 sm:h-48 md:h-52 flex items-center justify-center p-2 sm:p-3 bg-gray-50">
                    <Image src={post.image} alt={post.title} fill className="object-contain p-2 transition-transform duration-300 group-hover:scale-[1.03]" />
                ...
                <div key={post.id} className="rounded-2xl p-4 sm:p-6 md:p-8 shadow-[8px_3px_22px_10px_#9696961C] bg-white">
                  <div className={`grid md:grid-cols-2 gap-6 md:gap-8 items-center`}>
                    <div className={`${imageFirst ? '' : 'md:order-2'}`}>
                      <div onClick={() => openPost(post)} className="w-full h-56 sm:h-64 md:h-80 rounded-xl flex items-center justify-center p-2 sm:p-3 bg-gray-50 cursor-pointer group relative">
                        <Image src={post.image} alt={post.title} fill className="object-contain p-2 transition-transform duration-300 group-hover:scale-[1.02]" />
                      </div>
                    </div>
          ...
          <div className="absolute inset-0 flex items-center justify-center px-3 sm:px-4">
            <div className="bg-white w-full max-w-3xl rounded-2xl overflow-hidden shadow-xl">
              ...
              <div className="p-4 sm:p-6">
                ...
                <div className="w-full h-56 sm:h-64 md:h-72 bg-gray-50 rounded-xl flex items-center justify-center p-2 sm:p-3 mb-4 relative">
                  <Image src={activePost.image} alt={activePost.title} fill className="object-contain p-2" />
                </div>
